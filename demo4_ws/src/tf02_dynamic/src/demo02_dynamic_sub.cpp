...
int main(int argc, char *argv[])
{
    ...
    // 3-2.再创键监听对象(监听对象可以将订阅的数据存入 buffer )
    ...
    // ------------- 修改2 ------------------
    // 时间戳
    ps.header.stamp = ros::Time(0.0); // 0.0相当于不加时间戳，直接转换成不判断时间是否对齐
    ...
    // 方案1：添加休眠:以防已经开始订阅了，而发布者还没有发布消息
    ...
    while (ros::ok())
    {
        // 核心代码 --- 将 ps 转换成相对于 base_link的坐标点
