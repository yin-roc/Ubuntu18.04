...
int main(int argc, char *argv[])
{
    ...
    // ros::Duration类 的toSec()函数的原型如下: double toSec() const;
    ...
    // 2、时刻与时刻却可以相减
    ros::Duration du2 = begin - stop;   // 注意此处相减后的类型不再是Time类型
    ...
    ros::Timer createTimer  (ros::Duration period,              // 时间间隔 ---- 1s
                            ...
                            bool oneshot = false,               // 是否是一次性，如果是ture，则隔1s执行一下回调函数，只执行一次
                                                                // 如果是false，每隔1s执行以下回调函数
