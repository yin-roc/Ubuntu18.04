...
// 3.1.5 其他函数中程序中的其他部分调用了节点关闭相关的API示例
// (C++中是ros::shutdown()，python中是rospy.signal_shutdown())
...
int main(int argc, char * argv[]) 
{
    ...
    /*
        ...
        使用：
            1、argc 与 argv 的使用
                如果按照 ros 中的特定格式传入实参，那么ros可以加以使用，比如用来设置全局参数、给节点命名......
                exp:
                    1.demo03_ws rosrun plumbing_apis demo01_apis_pub _length:=2
                    2.rosparam list
                        /erGouzi/length
                        /rosdistro
            ...
            2、options 的使用
                ...
                结果：
                    ros当中有重名的节点启动时，之前的节点会被关闭
                需求：
                    特定场景下，需要一个节点多次启动，且能正常运行，怎么办？
                解决：
                    设置启动项：ros::init_options::AnonymousName
                    当创建ros节点时，会在用户自定义的节点名称后缀随机数，从而避免重名问题

                1、修改初始化函数为：
                        ros::init(argc, argv, "erGouzi", ros::init_options::AnonymousName); // 用的erGouzi作为前缀，但每次在后面加上一个随机数
    ...
    */
    ...
    /*
        ...
        参数：
            1、topic：话题名称
            2、queue_size：队列长度
            3、latch(可选)：布尔值，如果设置为true,它会保存发布方的最后一条消息，
        ...
        使用：
            ...
            以静态地图发布为例：
                ...
                方案2：可以将地图发布对象的 latch 设置为 true，并且发布方只发布一次数据，每当订阅者连接时，
